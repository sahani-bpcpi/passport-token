...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if(options && options.public_paths && (!username || !token)) {  
    for(var i = 0; i < options.public_paths.length; i++) {
      if(req.url.toLowerCase().indexOf(options.public_paths[i].toLowerCase()) === 0){
          this._verify(req, "public", "DtQpZEsiGC3HZ5JN0VRp8J1LH8nMMapoCBElJgeraT", verified);
          public_route = true;
      }
    }
  }
  
  if (!username || !token && !public_route) {
